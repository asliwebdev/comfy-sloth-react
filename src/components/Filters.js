...
const Filters = () => {
  const {filters: {text, company, category, color, max_price, min_price, price, shipping}, updateFilters, clearFilters, allProducts} = useFilterContext();
      ...
      <form onSubmit={(e) => e.preventDefault()}>
         ...
         <div className="form-control">
          ...
          <div>
            {categories.map((c, index) => {
              return <button key={index} type="button" name='category' onClick={updateFilters} className={`${category === c.toLowerCase() ? 'active' : null}`}>
                 {c}
              </button>
            })}
          </div>
         ...
         <div className="form-control">
          ...
          <div className="colors">
            {colors.map((c, index) => {
              if(c === 'all') {
                return <button key={index} name='color' data-color='all' className={`${color === 'all' ? 'all-btn active' : 'all-btn'}`} onClick={updateFilters}>all</button>
              }
